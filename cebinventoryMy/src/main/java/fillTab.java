 ...
 * @author Isuru
 */
public class fillTab extends HttpServlet {
...
List empList = session.createQuery("from owner_item_master  where user_name='"+name+"' and freeze='1'").list();
...
pw.println(oim.getItem_code()+":"+StatusConverter.convertDigitsToString(oim.getStatus())+":"+oim.getQuantity()+":"+im.getRemarks()+"%");
}
tx.commit();
}catch(Exception e){
    tx.rollback();
